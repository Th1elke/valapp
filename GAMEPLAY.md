...
- **Agendamento por dia da semana**: `habits.frequency` (`diaria` | `semanal` | `dias_customizados`) + `habits.customDays` (array de 0-6, domingo-sábado, igual `Date#getDay()`). Lógica central em `shared/habitSchedule.ts` — `isHabitScheduled(frequency, customDays, dateStr)` só decide se um **check-in individual** é permitido naquele dia (`checkin.post.ts` recusa com 400 se não). `dias_customizados` filtra por dia da semana; `diaria` e `semanal` sempre retornam `true` — pra `semanal` ("N vezes por semana, qualquer dia") isso é de propósito, já que qualquer dia serve pro check-in; a meta em si é avaliada em outro lugar, ver `habits.weeklyTarget` abaixo.
- **`semanal` (meta flexível)**: campo `habits.weeklyTarget` (1-7, só setado quando `frequency === 'semanal'`). Não é avaliado dia a dia — `server/utils/dailyClosure.ts` só considera um hábito `semanal` "elegível" no domingo (`isWeekEndDate`, `shared/date.ts`), contra a contagem de check-ins da semana inteira (segunda a domingo). Bateu a meta → não é punido, conta pra "dia perfeito" daquele domingo, sequência intocada. Não bateu → cai no mesmo `missed`/`punishableMissed` de um hábito diário perdido, reaproveitando toda a lógica de HP/streak/escudo/Segunda Voz sem duplicação. `habits/index.get.ts` calcula `weeklyProgress` (check-ins desde a última segunda) pra UI mostrar "X/N essa semana" (`habitos.vue`, `HabitCard.vue`).
  - Dashboard (`index.vue`) exclui hábitos `semanal` do cálculo local de "dia perfeito" (`dailyRatioHabits`) pelo mesmo motivo — eles continuam aparecendo em "Hábitos de hoje" (dá pra fazer check-in em qualquer dia) mas não entram na razão "X/Y hábitos de hoje", já que "não feito hoje" não significa nada pra eles isoladamente.
- **Pausa com datas** (docs seção 6): `pausedFrom`/`pausedUntil` em uso — `PATCH /api/habits/[id]` aceita `pausedUntil` opcional ao setar `status: 'pausado'` (`pausedFrom` vira a data de hoje automaticamente). `resumeExpiredPauses` (`server/utils/habits.ts`) retoma sozinho (`status` volta a `ativo`, datas limpas) assim que `pausedUntil` passa — sem cron, roda de forma preguiçosa sempre que os hábitos do usuário são tocados (mesmo padrão de `ensureWeeklyShield`): `habits/index.get.ts`, `checkin.post.ts`, início de `dailyClosure.ts`. O congelamento em si (sem XP/HP/streak) não precisou de lógica nova — `status = 'pausado'` já excluía o hábito do `where(habits.status = 'ativo')` usado em toda punição/check-in.
...
- ~~Hábitos semanais / dias customizados~~ — **resolvido** pros dois modos (`dias_customizados` e `semanal`), ver seção "Hábitos e Check-in".
- ~~Pausar hábito com datas~~ — **resolvido**, ver seção "Hábitos e Check-in".
