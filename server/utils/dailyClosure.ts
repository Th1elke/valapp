...
/**
 * Closes a given day for one user: applies HP loss for habits missed that day, or the
 * perfect-day HP/XP/gold bonus, then handles relapse (HP === 0) per docs/01-regras-gamificacao.md
 * sections 5 and 11. Idempotent via the daily_closures table (unique on user+date) — safe to call
 * more than once for the same user/date, whether from the manual "Fechar dia de ontem" button
 * (server/api/daily-closure.post.ts) or the Vercel Cron job (server/api/cron/daily-closure.get.ts).
 * Gold is never touched by relapse (section 11.6).
 */
export async function closeDayForUser(userId: string, closureDate: string) {
